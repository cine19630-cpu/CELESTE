...
public static class Audio
{
	public static class Banks
	{
		...
		private static string ResolveBankPath(string name)
		{
			try
			{
				var baseDir = Engine.ContentDirectory;
				var pAndroid = Path.Combine(baseDir, "FMOD", "Android", name);
				if (File.Exists(pAndroid + ".bank")) return pAndroid;
				var pDesktop = Path.Combine(baseDir, "FMOD", "Desktop", name);
				if (File.Exists(pDesktop + ".bank")) return pDesktop;
			}
			catch { }
			return null;
		}
		...
		public static Bank Load(string name, bool loadStrings)
		{
			...
			string text = ResolveBankPath(name);
				...
				{
					PortServices.Warn("CELESTE/AUDIO", $"BANK_NOT_FOUND: {name} (FMOD/Android ou FMOD/Desktop)");
					return default(Bank);
				}
	...
	private static bool ready;
		...
		public static bool IsReady => system != null && ready && !disabled;
	...
	public static void Init()
	{
		...
		catch (Exception ex)
		{
			...
			ready = false;
			system = null;
			PortServices.Exception("CELESTE/AUDIO", ex, "FMOD_INIT_FAILED");
		}
	}
	...
	{
		if (!IsReady || system == null) return false;
		bool paused = false;
	...
	{
		if (!IsReady || system == null) return 0f;
		VCA vca;
	...
	{
		if (!IsReady || system == null) return null;
