...
public class Engine : Game
{
	...
	public static string ContentDirectory
	{
		get
		{
			var p = PortServices.Paths?.ContentPath;
	...
	public Engine(int width, int height, int windowWidth, int windowHeight, string windowTitle, bool fullscreen, bool vsync)
	{
		...
		// Alguns targets do MonoGame (especialmente nas variações modernas de Android/.NET)
		// não expõem o mesmo método protegido virtual para override de encerramento.
		// Em vez de depender de override, usamos o evento Exiting + Dispose(bool).
		base.Exiting += OnGameExiting;
	}
	...
	protected override void Update(GameTime gameTime)
	{
		...
		FrameCounter++;
		MInput.Update();
			var boot = PortServices.Boot;
			if (boot != null && boot.IsBlockingGameLoop)
			{
	...
	{
		// Evita crash em shutdown e garante liberação de recursos de input.
		try
		{
			MInput.Shutdown();
		}
